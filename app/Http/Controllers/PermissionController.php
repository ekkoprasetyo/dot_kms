...
class PermissionController extends Controller
{
    ...
    public function store(PermissionRequest $request){
        ...
        try {
            $permission = new PermissionModel([
                ...
                'c_permission_update_by' => UserAuth::getUserID(),
                'c_permission_update_time' => date('Y-m-d H:i:s'),
                'c_permission_softdelete' => 0,
            ]);
            $permission->save();
            DB::commit();
    ...
    public function update(PermissionRequest $request){
        ...
        try {
            ...
            $permission->c_permission_update_by = UserAuth::getUserID();
    ...
    public function destroy(Request $request){
        ...
        try {
            //Update to Database
            $permission = PermissionModel::find($request->txt_permission_id);
            $permission->c_permission_softdelete = 1;
            $permission->c_permission_update_by = UserAuth::getUserID();
            $permission->c_permission_update_time = date('Y-m-d H:i:s');
            $permission->save();
            DB::commit();
